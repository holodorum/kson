...
object Kson {
     ...
     */
    fun analyze(kson: String, filepath: String = "") : Analysis {
        val parseResult = KsonCore.parseToAst(
            kson,
            CoreCompileConfig(validationContext = ValidationContext(filepath))
        )
...
class SchemaValidator internal constructor(private val schema: JsonSchema) {
     ...
     */
    fun validate(kson: String, filepath: String = ""): List<Message> {
        ...
        if (ksonValue != null) {
            schema.validate(ksonValue, messageSink, ValidationContext(filepath))
        }
