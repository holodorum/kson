...
object KsonTooling {
     ...
     *
     * @param document The pre-parsed document
     * @param schemaContent The schema source text (parsed strictly on each call
     *   to produce accurate source locations — schemas are expected to be valid)
    ...
    fun getSchemaInfoAtLocation(
        ...
        val context = ResolvedSchemaContext.resolveAndFilterSchemas(schemaContent, document.ksonValue, documentPointer) ?: return null
     ...
     * schema document. Filters schemas based on validation.
     *
     * @param document The pre-parsed document
     * @param schemaContent The schema source text (parsed strictly on each call
     *   to produce accurate source locations — schemas are expected to be valid)
    ...
    fun getSchemaLocationAtLocation(
        ...
        val context = ResolvedSchemaContext.resolveAndFilterSchemas(schemaContent, document.ksonValue, documentPointer) ?: return emptyList()
    ...
    fun resolveRefAtLocation(
        ...
        // Parse the schema strictly for correct source locations
        val parsedSchema = KsonCore.parseToAst(schema.content).ksonValue ?: return emptyList()
    ...
    fun getEnclosingRanges(document: ToolingDocument, line: Int, column: Int): List<Range> {
        ...
        if (last == null || last.startLine != documentRange.startLine || last.startColumn != documentRange.startColumn
            || last.endLine != documentRange.endLine || last.endColumn != documentRange.endColumn) {
    ...
    private data class ResolvedSchemaContext(
        ...
        companion object {
            /**
             * Common helper to resolve and filter schemas based on a document path.
             *
             * @param schemaContent The schema source text, parsed strictly to produce
             ...
             * @return ResolvedSchemaContext, or null if the schema has no parseable value
             */
            fun resolveAndFilterSchemas(
                ...
                val parsedSchema = KsonCore.parseToAst(schemaContent).ksonValue ?: return null
                val schemaIdLookup = SchemaIdLookup(parsedSchema)
 ...
 */
class DiagnosticMessage(val message: String, val severity: DiagnosticSeverity, val range: Range)
