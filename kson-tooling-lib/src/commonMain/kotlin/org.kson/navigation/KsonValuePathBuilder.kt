...
class KsonValuePathBuilder(
     ...
     * so the returned name matches what [AstNodeWalker.getObjectProperties]
     * returns via `processedStringContent`.
     */
    ...
    private fun adjustPathForLocationContext(
        ...
        val isAfterColonAtParent = colonPropertyName != null &&
                walker.isObject(targetNode) &&
                Location.containsCoordinates(walker.getLocation(targetNode), colonToken.lexeme.location.start)
        val tokens = when {
