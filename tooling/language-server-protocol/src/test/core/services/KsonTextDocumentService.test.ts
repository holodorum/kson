...
describe('KsonTextDocumentService', () => {
    ...
    describe('Semantic Tokens', () => {
        it('should provide semantic tokens for a document', async () => {
            ...
            // name: "value" produces tokens for: key 'name', operator ':', string '"value"'
            assert.ok((result as any).data.length >= 15, "Should have at least 3 tokens (5 ints each)");
        });
    ...
    describe('Document Highlight', () => {
        ...
        it('should return highlights for a document', async () => {
            ...
            // Cursor on 'name' key — should highlight sibling keys too
            assert.ok(result.length >= 1, "Should highlight at least the key under cursor");
        });
    });
    ...
    describe('Document Symbol', () => {
        ...
        it('should return symbols for a document', async () => {
            ...
            assert.ok(result[0].children.length >= 2, "Root object should have name and age children");
        });
    });
    ...
    describe('Folding Ranges', () => {
        it('should return empty array when document is not found', async () => {
            ...
            const result = await connection.foldingRangeHandler(params, {} as any, {} as any, undefined);
            assert.deepStrictEqual(result, []);
        });
        ...
        it('should return folding ranges for multi-line object', async () => {
            ...
            assert.ok(Array.isArray(result), "Should return an array");
            assert.strictEqual(result.length, 1, "Should have exactly one folding range for the object");
    ...
    describe('Selection Ranges', () => {
        it('should return empty array when document is not found', async () => {
            ...
            const result = await connection.selectionRangeHandler(params, {} as any, {} as any, undefined);
            assert.deepStrictEqual(result, []);
        });
        ...
        it('should return selection ranges for a document', async () => {
            ...
            const result = await connection.selectionRangeHandler(params, {} as any, {} as any, undefined);
            assert.ok(Array.isArray(result), "Should return an array");
            assert.strictEqual((result as any[]).length, 1, "Should have one selection range per position");
        });
    });
